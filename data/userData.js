...
async function getUserById(id) {
    // User_marketing_preferences join table with marketing_preferences -> many to many rs
    ...
    // Storing the specific user preferences into the user object
...
async function createUser({name, email, password, salutation, country, marketingPreferences}) {
    // marketingPreferences is in a many-to-many rs, means need to create the user first,
    // then insert the preferences
    let connection = null;
    try {

        connection = await pool.getConnection();
        // Beginning transaction so that can create a new row into 2 tables at the same time
        // Transaction in ACID DB: When performing 2 or more insert or update (count as 1 atomic process), 
        // but if 1 of them fail, need to revert (esp since the insert/update are dept on each other)
        ...
        // Because we get connection manually from the pool, need release when done
        connection.release();
    }
